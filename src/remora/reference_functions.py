...
class referenceEncoder:
    ...
    def reference_encoding_bybase(
        ...
        for sig, ref, bl in zip(signals, references, base_locs):
            extracted_bl = bl[
                self.chunk_info.mod_offset
                - self.chunk_info.chunk_bases[0] : self.chunk_info.mod_offset
                + self.chunk_info.chunk_bases[1]
                + 1
            ]
            extracted_ref = ref[
                self.chunk_info.mod_offset
    ...
    def reference_encoding_bychunk(
        ...
        for sig, ref, bl in zip(signals, references, base_locs):
            index_below = (
                bisect.bisect(
                    bl,
                    bl[self.chunk_info.mod_offset]
                    - self.chunk_info.fixed_chunk_size[0],
                )
                - 1
            )
            index_above = (
                bisect.bisect(
                    bl,
                    bl[self.chunk_info.mod_offset]
                    + self.chunk_info.fixed_chunk_size[1],
                )
                - 1
            )
            encoding = np.zeros(
