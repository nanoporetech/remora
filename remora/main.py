...
parser.add_argument('--chunk_bases', default=[], type=tuple,
...
parser.add_argument('--mod-offset', default = 20, type = int,
    dest = 'MOD_OFFSET',
    help = 'Seed value'
)
...
if __name__ == '__main__':
    ...
    if len(args.chunk_bases) ==0:
        sigs, labels, refs, base_locs = get_train_set(args.dataset_path,
        							 				  args.MOD_OFFSET)
    elif len(args.chunk_bases) == 1:
        if not isinstance(args.chunk_bases[0], int):
            raise ValueError('number of bases before and after mod base must be integer values')

        sigs, labels, refs, base_locs = sample_chunks(args.dataset_path,args.number_to_sample,
                                                        args.chunk_bases[0], args.chunk_bases[0],
                                                        args.MOD_OFFSET)
    else:
        if len(args.chunk_bases) > 2:
            print('Warning: chunk bases larger than 2, only using first and second elements')

        if not all(isinstance(i, int) for i in args.chunk_bases):
            raise ValueError('number of bases before and after mod base must be integer values')

        sigs, labels, refs, base_locs = sample_chunks(args.dataset_path,args.number_to_sample,
                                                        args.chunk_bases[0], args.chunk_bases[1],
                                                        args.MOD_OFFSET)
    ...
    for epoch in range(args.epochs):
        ...
        for x, len_x, y in dl_tr:
            x_pack = rnn.pack_padded_sequence(x.unsqueeze(2),
            								  len_x,
